...
metadata2 <- read.delim("jar_metadata_rep1.txt")
colnames(metadata) <- gsub("X","",colnames(metadata))

### Subtract the tryptone controls

PR_raw <- read.delim("OD_isolates_rep1.txt")
...
PR %>% mutate(concerning = ifelse(diff > 0.2, TRUE, FALSE)) %>%
  select(IsolateID, concerning)%>% View()

volume <- PR %>% mutate(volPerMl = 0.05/aveOD) %>%
  select(IsolateID, volPerMl) 
...
combinedVolumes
water_vol <- 4-colSums(combinedVolumes[,-which(colnames(combinedVolumes)%in%c("IsolateID","volPerMl"))])
