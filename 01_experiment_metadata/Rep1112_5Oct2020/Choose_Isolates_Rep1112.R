...
library(tidyverse)
setwd("./Rep1112_5Oct2020/")
### Inhibitory Sampling ####
inhibDat <- read.delim("../../IsolateInfo/all_isolate_info_combined.txt")
inhibToKeep <- read.delim("../../IsolateInfo/Inhib_to_include.txt", header=FALSE)$V1
noninhibToKeep <- read.delim("../../IsolateInfo/nonInhib_to_include.txt", header=FALSE)$V1

inhibDat_filt <- inhibDat %>% filter(BiofilmFormer, !is.na(Inhibitory)) %>%
  filter(IsolateID %in% c(inhibToKeep, noninhibToKeep))

date_start <- c("2020-10-05")
njars <- 30
jarNames <- 1:28
RichLevels <- c(1,3,10)
...
##### INHIB #####
set.seed(6234)
### Different this time because I want to include ones that I'd missed previously
have <- c("13C","54D","45D","BTB_59","32E","34B","25A")

inhibDat_inhib1 <- inhibDat_filt %>% filter(Inhibitory, BiofilmFormer) %>%
  mutate(randomOrder = sample(1:nrow(.), replace=FALSE)) %>%
  mutate(randomOrder = ifelse(IsolateID %in% have, 0, randomOrder)) %>%
  arrange(randomOrder) %>% 
  ...
  mutate(L0 = NA,L1 = c(rep(RichLevels[1], RichLevels[1]), rep(NA, n()-RichLevels[1])), L2 = c(rep(RichLevels[2], RichLevels[2]), rep(NA, n()-RichLevels[2])), L3=c(rep(RichLevels[3], RichLevels[3]), rep(NA, n()-RichLevels[3]))
         , Inhibitory="Inhib", Rep="Rep11", DateStart=date_start) 
inhibDat_inhib2 <- inhibDat_filt %>% filter(Inhibitory, BiofilmFormer) %>%
  ...
  mutate(randomOrder = 1:nrow(.)) %>%
  mutate(L0 = NA,L1 = c(rep(RichLevels[1], RichLevels[1]), rep(NA, n()-RichLevels[1])), L2 = c(rep(RichLevels[2], RichLevels[2]), rep(NA, n()-RichLevels[2])), L3=c(rep(RichLevels[3], RichLevels[3]), rep(NA, n()-RichLevels[3]))
         , Inhibitory="Inhib", Rep="Rep12", DateStart=date_start) 
...
inhibDat_notinhib1 <- inhibDat_filt %>% filter(!Inhibitory, BiofilmFormer) %>%
  ...
  mutate(randomOrder = 1:nrow(.)) %>%
  mutate(L0 = NA,L1 = c(rep(RichLevels[1], RichLevels[1]), rep(NA, n()-RichLevels[1])), L2 = c(rep(RichLevels[2], RichLevels[2]), rep(NA, n()-RichLevels[2])), L3=c(rep(RichLevels[3], RichLevels[3]), rep(NA, n()-RichLevels[3]))
         , Inhibitory="Non", Rep="Rep11", DateStart=date_start) 
inhibDat_notinhib2 <- inhibDat_filt %>% filter(!Inhibitory, BiofilmFormer) %>%
  ...
  mutate(randomOrder = 1:nrow(.)) %>%
  mutate(L0 = NA,L1 = c(rep(RichLevels[1], RichLevels[1]), rep(NA, n()-RichLevels[1])), L2 = c(rep(RichLevels[2], RichLevels[2]), rep(NA, n()-RichLevels[2])), L3=c(rep(RichLevels[3], RichLevels[3]), rep(NA, n()-RichLevels[3]))
         , Inhibitory="Non", Rep="Rep12", DateStart=date_start) 
...
agg_dat_filt <- agg_dat %>% filter(!(Rep %in% c("Rep11","Rep12")))
...
## Write out
metadata %>%
  write.table(file="jar_metadata_rep1112.txt", sep="\t", quote=FALSE, row.names=FALSE)
